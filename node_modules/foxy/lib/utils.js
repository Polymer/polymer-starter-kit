...
utils.rewriteLinks = function rewriteLinks(userServer, proxyUrl) {
    ...
    return {
        ...
        fn:    function (match) {
            ...
            /**
             * If we're looking at an escaped url, just replace the hostname and return
             */
            if (match.indexOf("\\") > -1) {
                return match.replace(string, proxyUrl);
            }

            var capturedQuote = (match[0] === "'" || match[0] === "\"") ? match[0] : "";


            /**
             * If the first char of a match was a quote, slice it off
             */
            if (match[0] === "'" || match[0] === "\"") {
                match = match.slice(1);
            }
            ...
            if (!out.host) {
                ...
                return capturedQuote + match.replace(string, proxyUrl);
            }
